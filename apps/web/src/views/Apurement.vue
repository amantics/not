...
const handleEnding = (item: TTransaction) => {
  item.ended = true;
  //  fix dropdown menu not hiding after click
  addNotification?.({
    ref: item.ref,
    message: `Notaire a apuré le transaction.`,
  ...
  <main>
    ...
    >
      ...
      <template #cell(ended)="data">
        <b-badge v-if="data.value" pill variant="info">apurée</b-badge>
        <b-button v-else @click="handleEnding(data.item)" variant="primary"
          >Apurer</b-button
        >
      </template>
    </b-table>
