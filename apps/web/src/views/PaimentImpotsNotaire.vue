...
const handleInitialize = (item: TTransaction) => {
  item.taxPaymentStatus = "INITIALIZED";
  addNotification?.({
    ref: item?.ref,
    message: `Un notaire a initialisé le paiement d'impôt`,
    intendedTo: convertRolesToIndexes(["CDG"]),
    paths: {
      CDG: "paiement-impots-cdg",
    },
  });
};
</script>
  ...
  <main>
    ...
    >
      ...
      <template #cell(taxPaymentFile)="data">
        <b-button
          variant="link"
          v-if="!!data.item.taxPaymentStatus"
          :href="data.value?.name ?? '#'"
          @click.prevent="downloadFile(data.value)"
          :disabled="
            data.item.taxPaymentStatus === 'INITIALIZED' || !data.value
          "
        ...
        <b-badge v-else-if="data.item.ended" pill variant="info"
          >apurée</b-badge
        >
        <b-button variant="primary" v-else @click="handleInitialize(data.item)">
          Initialiser</b-button
        >
      </template>
